...
# master
["master", "slave"].each do |t|
 data = YAML.load(IO.readlines("user-data.#{t}.sample")[1..-1].join)
 if data['coreos'].key? 'etcd2'
   data['coreos']['etcd2']['discovery'] = token
 end

 # Fix for YAML.load() converting reboot-strategy from 'off' to `false`
 if data['coreos'].key?('update') && data['coreos']['update'].key?('reboot-strategy')
 ...
 yaml = YAML.dump(data)
 File.open("user-data.#{t}", 'w') { |file| file.write("#cloud-config\n\n#{yaml}") }
end
